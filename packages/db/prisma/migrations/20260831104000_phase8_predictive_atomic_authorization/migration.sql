CREATE FUNCTION lock_predictive_evidence_authorization(
...
BEGIN
  ...
  IF (p_membership_id IS NULL) = (p_support_access_id IS NULL) THEN
    RETURN false;
  END IF;
...
REVOKE ALL ON FUNCTION lock_predictive_evidence_authorization(UUID, UUID, UUID, UUID, UUID, BOOLEAN)
FROM PUBLIC;
GRANT EXECUTE ON FUNCTION lock_predictive_evidence_authorization(UUID, UUID, UUID, UUID, UUID, BOOLEAN)
TO jormall_app;
