CREATE OR REPLACE FUNCTION protect_predictive_job_relay_envelope() RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
  reclaimable BOOLEAN;
BEGIN
  ...
  reclaimable :=
    ...
    OR (OLD."status" = 'CLAIMED'
        AND OLD."claimed_at" < statement_timestamp() - INTERVAL '2 minutes')
    OR (OLD."status" = 'ENQUEUED'
        AND OLD."enqueued_at" < statement_timestamp() - INTERVAL '5 minutes')
    OR (OLD."status" = 'RUNNING'
        AND OLD."started_at" < statement_timestamp() - INTERVAL '30 minutes');
